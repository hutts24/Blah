...
void blah_input_keyboard_sdl_init () { //initialises SDL keyboard input
	...
	if (!SDL_WasInit(SDL_INIT_VIDEO)) //if SDL Video hasn't been initialised
		blah_video_init(); //Initialise SDL with video subsystem and we get keyboard also

	fprintf(stderr,"initing mappings\n");

	//Set all mappings to -1, meaning not mapped to engine key symbols
	for (symCount=0; symCount < SDLK_EURO; symCount++)
		blahInputKeyboardSDLMap[symCount] = -1;
