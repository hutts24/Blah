...
{	//Initialise an entity structure using supplied name and object pointer.
	...
	entityObject->visible = BLAH_TRUE;
...
blah_bool Blah_Entity_Object_isVisible(Blah_Entity_Object *entityObject)
...
void Blah_Entity_Object_setVisible(Blah_Entity_Object *entityObject, blah_bool visFlag) {
...
blah_bool Blah_Entity_Object_checkCollision(Blah_Entity_Object *entityObject1,
	...
	float distance;
	blah_bool colliding = BLAH_FALSE;
	...
	if (distance < object1->boundRadius + object2->boundRadius) {
		...
		if (Blah_Entity_Object_distanceObject(&dummyObject, entityObject2) < distance) {
			colliding = BLAH_TRUE;  //If distance next frame is less,
	...
	//return distance < object_1->bound_radius + object_2->bound_radius?BLAH_TRUE:BLAH_FALSE;
	return colliding;
}
